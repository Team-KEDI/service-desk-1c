...
Процедура ВРаботуНаСервере()
	...
	КонецЕсли;
	Если Не СтатусВРаботе.Пустая() И (Объект.Статус = СтатусНазначено ИЛИ Объект.Статус = СтатусВозвратВРаботу) Тогда
...
Процедура РешеноНаСервере()
	...
	Если Не СтатусНаПодтверждении.Пустая() И Объект.Статус = СтатусВРаботе Тогда
...
Процедура ПринятьРешениеНаСервере()
	...
	Если Не СтатусНаПодтверждении.Пустая() И Объект.Статус = СтатусНаПодтверждении Тогда
		ДокументОбъект.Статус = СтатусРешено;
		
		Попытка
			ДокументОбъект.Записать();
			// ЧИТАЕМ данные из базы обратно на форму, чтобы статус обновился на экране
			ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
			Сообщить("Статус обновлен: Решено");
		Исключение
			Сообщить("Ошибка при записи: " + ОписаниеОшибки());
		КонецПопытки;
	КонецЕсли;
...
Процедура ОтклонитьРешениеНаСервере()
	...
	Если Не СтатусНаПодтверждении.Пустая() И Объект.Статус = СтатусНаПодтверждении Тогда
		ДокументОбъект.Статус = СтатусВозвратВРаботу;
		
		Попытка
			ДокументОбъект.Записать();
			// ЧИТАЕМ данные из базы обратно на форму, чтобы статус обновился на экране
			ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
			Сообщить("Статус обновлен: Возврат в работу");
		Исключение
			Сообщить("Ошибка при записи: " + ОписаниеОшибки());
		КонецПопытки;
	КонецЕсли;
