...
Процедура ВРаботуНаСервере()
	...
	// Используем ссылку напрямую из данных формы
	// Если реквизит называется "Объект" и имеет тип "ДокументСсылка"
	Если Объект.Ссылка.Пустая() Тогда
		// Если документ еще не записан в базу, его нужно сначала записать
		ЭтотОбъект.Записать(); 
	КонецЕсли;
	
	// Получаем полноценный объект из базы по ссылке
